...
export default function SalesPosPage() {
  ...
  const cartKeyName = `balaji_pos_cart_${userId}`;
  const vehicleKeyName = `balaji_pos_vehicle_${userId}`;

  const [selectedVehicleId, setSelectedVehicleId] = useState(() => {
    return localStorage.getItem(`balaji_pos_vehicle_${userId}`) || localStorage.getItem('pepsi_pos_vehicle') || '';
  });
  ...
  // Persistent User-Isolated Cart state stored in localStorage across page navigation
  const [cart, setCart] = useState(() => {
    try {
      const saved = localStorage.getItem(`balaji_pos_cart_${userId}`);
  ...
  useEffect(() => {
    if (user?._id) {
      localStorage.setItem(`balaji_pos_cart_${user._id}`, JSON.stringify(cart));
    }
  }, [cart, user?._id]);
  ...
  useEffect(() => {
    if (user?._id) {
      try {
        const saved = localStorage.getItem(`balaji_pos_cart_${user._id}`);
  ...
  // Save selected vehicle to user-isolated localStorage
  useEffect(() => {
    if (selectedVehicleId && user?._id) {
      localStorage.setItem(`balaji_pos_vehicle_${user._id}`, selectedVehicleId);
    }
  ...
  const addToCart = (itemObj, unitType = 'Box') => {
    if (!selectedCustomerId) {
      toast.warning('Please select a customer first', 'Customer Required');
      return;
    }

    const product = itemObj.product || itemObj;
    ...
    // Check stock available on the selected Van
    const vStock = vanStock.find(v => (v.product?._id || v.product) === product._id);
    const availVanBoxes = vStock ? Number(vStock.quantity || 0) : 0;
    const availVanHangers = availVanBoxes * hangersPerBox;

    // Calculate current requested hangers in cart for this product
    let currentCartHangers = 0;
    cart.forEach(c => {
      if (c.product._id === product._id) {
        const itemHangers = c.unitType === 'Hanger' ? c.quantity : (c.quantity * hangersPerBox);
    ...
    // ⛔ BLOCK ADDING TO CART IF INSUFFICIENT VAN STOCK
    if (newTotalRequestedHangers > availVanHangers) {
      toast.warning(
        `⛔ Insufficient Van Stock for "${product.name}"! Available on Van: ${availVanBoxes} Boxes (${availVanHangers} Hangers). Cannot add more ${unitType}s.`,
        'Van Stock Limit Reached'
      );
      return;
    }
  ...
  const updateCartQty = (cartKey, newQty) => {
    ...
    if (existingIndex > -1) {
      ...
      if (totalNewHangers > availVanHangers) {
        toast.warning(
          `⛔ Van Stock Limit Exceeded! Only ${availVanBoxes} Boxes (${availVanHangers} Hangers) available on Van for "${product.name}".`,
          'Van Stock Limit Reached'
        );
        return;
      }
  ...
  const handleClearCart = () => {
    if (window.confirm('Clear all items from POS cart?')) {
      setCart([]);
      if (user?._id) {
        localStorage.removeItem(`balaji_pos_cart_${user._id}`);
      }
      localStorage.removeItem('pepsi_pos_cart');
    }
  };
  ...
  // Extract all unique categories dynamically from Van Stock
  const posCategories = ['All', ...Array.from(new Set(vanStock.map(st => st.product?.category).filter(Boolean)))];

  const filteredStock = vanStock.filter(st => {
    if (!st || !st.product) return false;
    const prodCategory = st.product.category || 'Namkeen';
    const matchesCat = selectedCategory === 'All' || prodCategory === selectedCategory;
    const prodName = st.product.name ? String(st.product.name).toLowerCase() : '';
      ...
      <div className="bg-gradient-to-r from-[#0051A5] via-blue-800 to-[#002B66] text-white p-4 rounded-2xl shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-[#E31E24]">
        <div className="flex items-start sm:items-center space-x-3">
          <div className="p-3 bg-white/10 rounded-xl backdrop-blur-md shrink-0 mt-0.5 sm:mt-0">
            <Truck className="w-6 h-6 text-[#FFC72C]" />
          </div>
          <div className="space-y-1 min-w-0 flex-1">
            <h1 className="text-lg sm:text-xl font-black tracking-tight leading-tight flex items-center space-x-2">
              <span>Van Route Sales POS</span>
            ...
            <p className="text-xs text-blue-100 font-medium">
              Live loaded route billing & delivery for Boxes & Loose Hangers
            </p>
            ...
            <div className="pt-2 border-t border-white/10 flex flex-col md:flex-row md:items-center gap-1.5 md:gap-6 text-xs font-semibold text-blue-100">
              ...
              <div className="flex items-center space-x-1.5">
                <span className="opacity-80">Boxes Loaded:</span>
                <span className="font-extrabold text-white bg-white/15 px-2 py-0.5 rounded-md text-[11px]">
                  {selectedVehicleObj?.totalStockUnits || 0} Boxes
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-extrabold text-sm text-slate-900 dark:text-white flex items-center space-x-2">
                ...
                <span>Van Loaded Items ({filteredStock.length} SKUs)</span>
              </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filteredStock.map((st) => {
                ...
                const availStock = st.quantity || 0;
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between text-xs pt-1">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-[10px] font-extrabold text-blue-600 dark:text-blue-400">{availStock} Boxes on Van</span>
                        </div>
                      </div>
                    </div>
              ...
              {filteredStock.length === 0 && (
                <div className="col-span-2 py-12 text-center text-slate-400 italic text-xs space-y-1">
                  ...
                  <p>Go to "Van Loading" page to transfer items from warehouse to this van.</p>
                </div>
              )}
            </div>
          </div>
            ...
            <button
              ...
              className={`w-full py-4 font-black text-sm rounded-2xl transition flex items-center justify-center space-x-2 mt-2 ${
                cart.length === 0 || !selectedCustomerId || !selectedVehicleId || isSubmitting
                  ...
                  : 'bg-gradient-to-r from-[#DC2626] to-blue-600 hover:from-blue-700 hover:to-blue-800 text-white shadow-xl shadow-blue-500/25 active:scale-95 cursor-pointer'
              }`}
            >
              <span>
                {!selectedVehicleId 
